...
/** Traduz os dois preços na hora: é aqui que se percebe um desconto digitado errado. */
function AnnualPreview({
  ...
  const savedCents = monthlyCents * 12 - yearlyCents;
  const equivalent = Math.round(yearlyCents / 12);
  const over = savedCents < 0;
  ...
  return (
    ...
    >
      <p className={cn("text-caption", over ? "text-attention" : "text-ink-secondary")}>
        {over ? (
          <>
            No anual a conta paga {formatBRL(-savedCents)} a MAIS por ano do que pagando mês a mês.
            Confira os valores.
          </>
        ) : savedCents === 0 ? (
          <>O anual sai igual ao mensal: {formatBRL(equivalent)} por mês, sem desconto.</>
        ) : (
          <>
            No anual: {formatBRL(equivalent)} por mês —{" "}
            {formatPercent(savedCents / (monthlyCents * 12))} de desconto,{" "}
            {formatMonths(savedCents / monthlyCents)} de graça.
          </>
        )}
      </p>
...
// Gêmeos dos helpers da página. Não dá para compartilhar por import: este
// módulo é "use client" e a página é servidor — o que ela importaria daqui
// viria como referência de cliente, não como função.
