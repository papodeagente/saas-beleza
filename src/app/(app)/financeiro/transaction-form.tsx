...
export function TransactionForm({ branches }: { branches: Array<{ id: number; name: string }> }) {
  ...
  return <><Button onClick={() => setOpen(true)}><Plus />Novo lançamento</Button>{open ? <Sheet open onOpenChange={setOpen}><SheetContent title="Novo lançamento" description="Registre receitas e despesas que não nasceram de um atendimento." footer={<><Button variant="ghost" onClick={() => setOpen(false)}>Cancelar</Button><Button loading={pending} disabled={!description.trim() || !amount} onClick={() => document.getElementById("transaction-submit")?.click()}>Salvar lançamento</Button></>}>
