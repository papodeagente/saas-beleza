 ...
 *
 * Aqui é passagem, não destino: o pagamento acontece em `/pagar/<token>`, e a
 * tela vai para lá sozinha. O bilhete fica como rede de segurança, porque a
 * navegação pode demorar ou falhar, e quem acabou de escolher horário não pode
 * ficar olhando um formulário sem saber se deu certo.
 *
 * O que ele NÃO faz é dizer "confirmado": numa clínica que cobra, o horário
 * está guardado, e prometer confirmação aqui seria a mentira mais cara desta
 * tela.
 */
function BilheteAPagar({
  ...
  useEffect(() => {
    router.push(cobranca.caminho);
  }, [router, cobranca.caminho]);
    ...
    <main className="min-h-dvh bg-[radial-gradient(circle_at_top_left,#f1e5fb_0,transparent_36%),var(--color-surface)] px-5 py-10 sm:py-16">
      <div className="mx-auto max-w-[560px] overflow-hidden rounded-overlay bg-surface-raised shadow-[0_28px_80px_rgb(67_35_88/0.16)]">
        <div className="bg-brand px-7 py-8 text-white sm:px-10">
          ...
          <p className="mt-6 text-eyebrow text-white">Falta o pagamento</p>
          <h1 className="mt-2 text-display text-white">Seu horário está guardado</h1>
          <p className="mt-2 text-body text-white">
            {organizationName} guarda este horário por {cobranca.minutos} minutos, até o pagamento
            entrar.
          </p>
        </div>
        <div className="px-7 py-7 sm:px-10">
          <Button variant="primary" size="lg" className="w-full" asChild>
            <a href={cobranca.caminho}>
              <CreditCard className="size-4" aria-hidden />
              Pagar {cobranca.valorLabel}
            </a>
          </Button>
