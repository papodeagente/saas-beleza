...
const COLORS = ["#9966CC", "#955ECD", "#9157CD", "#8C4FCB", "#8744CD", "#712FB5"];
...
export function FunnelChart({ stages }: { stages: FunnelStageMetric[] }) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(310px,.65fr)]">
      <div className="relative flex min-h-[420px] flex-col justify-center overflow-hidden rounded-overlay border border-line bg-[radial-gradient(circle_at_50%_0%,rgba(153,102,204,.13),transparent_58%),var(--color-surface-sunken)] px-4 py-8 sm:px-8">
        ...
        <div className="relative mx-auto flex w-full max-w-[760px] flex-col items-center gap-1.5">
          {stages.map((stage, index) => {
            ...
            return (
              ...
              >
                <span className="absolute inset-0 shadow-[0_8px_24px_rgba(78,34,119,.16)] [clip-path:polygon(4%_0,96%_0,90%_100%,10%_100%)]" style={{ background: `linear-gradient(90deg,${COLORS[index]}dd,${COLORS[index]})` }} />
        ...
        {stages.map((stage, index) => (
          <li key={stage.key} className="relative">
            ...
            <button type="button" onMouseEnter={() => setActive(index)} onMouseLeave={() => setActive(null)} className={cn("flex w-full items-center gap-3 rounded-card border bg-surface-raised px-4 py-3 text-left shadow-card transition-colors", active === index ? "border-accent/45" : "border-line")}>
              <span className="size-2.5 shrink-0 rounded-full" style={{ background: COLORS[index] }} />
